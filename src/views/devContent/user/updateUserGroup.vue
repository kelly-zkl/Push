  ...
  <section class="app-man">
    <div class="main-content">
          ...
          <div class="form-add">
            <div class="fa-right">
              <div class="tag-list">
                ...
                <el-button class="button-new-tag" size="small" @click="chooseCity">选择省市</el-button>
              </div>
          ...
          <div class="form-add">
            <el-form-item label="">
              ...
              <span class="day-active">
								<el-input v-model="form.activeDays" :maxlength="10"
            ...
            <el-form-item label="">
              <div>
                <el-checkbox-group v-model="form.opsys">
                  <el-checkbox label="b29a89dc975c4ebb86c03c19d29b3260" name="type"
                               @change="handleAndroidChange">Android
                  </el-checkbox>
                  <el-checkbox label="f43a68bfd37b4cb3823a2efaf9eed6c0" name="type"
                               @change="handleIosChange">Ios
                  </el-checkbox>
                  <el-checkbox label="1f110e5787c64ebd86bbd1f436d9e17b" name="type"
                               @change="handleWindowsChange">Windows
                  </el-checkbox>
                </el-checkbox-group>
              </div>
              <el-row :gutter="20">
                <el-col :span="6">
                  <div class="grid-content bg-purple">
                    <div><img src="../../../assets/images/android_1.png"><span>Android</span></div>
                    <div><img src="../../../assets/images/ios_1.png"><span>Ios</span></div>
                    <div><img src="../../../assets/images/wp_1.png"><span>Windows</span></div>
                  </div>
                </el-col>
                <el-col :span="9">
                  <div class="grid-content bg-purple">
      ...
      <div class="text-right">
        ...
        <el-button type="primary" @click="onSubmit" size="large" :disabled=!isPress>确认修改</el-button>
      </div>
    </div>
  </section>
</template>
...
//        this.getCity('', 'pov');
          ...
          if (valid) {
            ...
            if (this.dynamicTags.length > 0 || this.form.cityIds.length > 0 || this.form.activeType
              ...
              for (let i in this.form.cityIds) {
                ...
                para.cityIds.push(this.form.cityIds[i].id || this.form.cityIds[i].cityId);
              }
              ...
              for (let i in this.form.opsys) {
                if (this.form.opsys[i] === this.androidInfo.opsysId) {
                ...
                if (this.form.opsys[i] === this.iosInfo.opsysId) {
                ...
                if (this.form.opsys[i] === this.windowInfo.opsysId) {
              ...
              para.groupDesc += '的用户';
              globalAxios('group/update', para).then((res) => {
                let data = res.data;
                this.isPress = true;
                if (data.code === '000000') {
      ...
      getInfoById() {
        globalAxios('group/get/' + this.form.groupId, {}).then((res) => {
          let data = res.data;
          if (data.code === '000000') {
            let obj = data.data;
            if (obj.tagsType) {
              this.form.tagsType = obj.tagsType + '';
            }
            this.form.groupName = obj.groupName;
            if (!!obj.activeType) {
              this.form.activeType = obj.activeType + '';
            }
            this.form.activeDays = obj.activeDays + '';

            this.form.appId = obj.appId || '';
            if (!!obj.groupTags) {
              this.dynamicTags = obj.groupTags.split(',');
            }
            for (let i in obj.groupOpsysBOs) {
              if (obj.groupOpsysBOs[i].opsysId === this.windowInfo.opsysId) {
                this.windowInfo = obj.groupOpsysBOs[i];
                this.isWindows = true;
                this.form.opsys.push(obj.groupOpsysBOs[i].opsysId);
              }
              if (obj.groupOpsysBOs[i
                      ].opsysId === this.androidInfo.opsysId) {
                this.androidInfo = obj.groupOpsysBOs[i];
                this.isAndroid = true;
                this.form.opsys.push(obj.groupOpsysBOs[i].opsysId);
              }
              if (obj.groupOpsysBOs[i].opsysId === this.iosInfo.opsysId) {
                this.iosInfo = obj.groupOpsysBOs[i];
                this.isIos = true;
                this.form.opsys.push(obj.groupOpsysBOs[i].opsysId);
              }
            }
            this.form.cityIds = obj.groupCityBOs || '';
            for (let i in this.form.cityIds) {
              this.form.cityIds[i]['id'] = this.form.cityIds[i].cityId;
              this.colorList[this.form.cityIds[i].cityId] = this.form.cityIds[i].cityId;
            }
          } else {
            this.$message.error(data.msg);
          }
